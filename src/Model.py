...
class Model: 
    ...
    def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False, user='', dump=False):
    ...
    def setupCTC(self):
        ...
        if self.decoderType == DecoderType.BestPath:
            ...
            wordChars = open(os.path.join('../model/', self.user)+'/wordCharList.txt').read().splitlines()[0]
            corpus = open(os.path.join('../data/', self.user)+'/corpus.txt').read()
    ...
    def setupTF(self):
        ...
        modelDir = os.path.join('../model/', self.user)
        
        #charger le dernier checkpoint
        latestSnapshot = tf.train.latest_checkpoint(modelDir) # there is a saved model?
    ...
    def dumpNNOutput(self, rnnOutput):
        ...
        #dumpDir = '../model/dump/'
        dumpDir=os.path.join('../model/', self.user)+'/dump/'
...
###
    def save(self):
        "save model to file"
        self.snapID += 1
        self.saver.save(self.sess, os.path.join(os.path.join('../model/',self.user),'snapshot'), global_step=self.snapID)
